...
public class dinnerTable : MonoBehaviour
{
    ...
    public  bool emptyDinnerTable = true;
    public Transform chairPosition;

    //ray ile gelen müþteriden bilgi çekme.
    public GameObject detectorRay;
    int customerLayerMask;
    ...
    void Start()
    {
        customerLayerMask = LayerMask.GetMask("customer");
        burgerOrder.SetActive(false);
    }
    // Update is called once per frame
    void Update()
    {
        //müþteri oturunca data çeker.
        Reset();
        //ýd kontrolü
        IDdetector();
        IDcheck();
    ...
    private void Reset()
    {
        Debug.DrawRay(detectorRay.transform.position, detectorRay.transform.forward * hitRange, Color.red);
        Ray ray = new Ray(detectorRay.transform.position, detectorRay.transform.forward);

        if (Physics.Raycast(ray, out RaycastHit hit, hitRange, customerLayerMask))
        {
            if (hit.transform.gameObject.TryGetComponent<musteriAI>(out var musteriAI))
            {
                if (musteriAI.order == true)
                {
                    if (!emptyDinnerTable)
                    {
                        if (counterID == 124)
                        {
                            ...
                            {
                                eatDelay = 0;
                                musteriAI.finishedEat = true;
                                counterID = 2;;
                            }
                        }
                        else
                        {
                            burgerOrder.SetActive(true);
                        }
                    }
                }
                else
