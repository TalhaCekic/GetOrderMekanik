...
public class DeliveryOrder : NetworkBehaviour
{
    ...
    [SyncVar] public bool ekranKart�Dolu = false;
    ...
    void Update()
    {
        if (isServer)
        {
            Rpc�dCheck(currentobjectnumber);
           // rpc(currentobjectnumber);
        }
        else
        {
            cmd�dCheck(currentobjectnumber);
    ...
    [Command(requiresAuthority = false)]
    void cmd�dCheck(int currentobjectnumber)
    {
        //�dCheck(managerOrder);
        Rpc�dCheck(currentobjectnumber);
 if (Time.time - lastResetTime > resetDelay)
    ...
    [ClientRpc]
    void Rpc�dCheck(int currentobjectnumberr)
    {
        �dCheck(currentobjectnumber);
    }
    public void �dCheck(int currentobjectnumber)
    {
        ...
        if (kasaDolu || anaKartDolu || cpuDolu || ekranKart�Dolu || ramDolu)
        {
            if (kasaDolu)
            {
                if (isServer)
            ...
            if (kasaDolu && anaKartDolu && ekranKart�Dolu)
            {
                if (isServer)
                {
                    RpcinteractID(124);
            ...
            if (kasaDolu && anaKartDolu && ekranKart�Dolu && ramDolu)
            {
                if (isServer)
                {
                    RpcinteractID(1245);
            ...
            if (kasaDolu && anaKartDolu && cpuDolu && ekranKart�Dolu)
            {
                if (isServer)
                {
                    RpcinteractID(1234);
    ...
    // bool de�i�kenlerin sunucuya g�nderilmesi
    [Command(requiresAuthority = false)]
    ...
    [Command(requiresAuthority = false)]
    public void CmdSetEkranKart�Dolu(bool newValue)
    {
        ekranKart�Dolu = newValue;
    }
    [Command(requiresAuthority = false)]
